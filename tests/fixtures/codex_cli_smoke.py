...
with tempfile.TemporaryDirectory(prefix='ccsw-cli-smoke-') as root:
 ...
 class Handler(http.server.BaseHTTPRequestHandler):
  ...
  def do_POST(self):
   ...
   response={'id':f'resp_{step}','object':'response','created_at':0,'model':'test-model','status':'completed','output':[item],'usage':{'input_tokens':1,'output_tokens':1,'total_tokens':2}}
   ...
   if kind=='anthropic':
    ...
    events=[{'type':'message_start','message':{'id':f'msg_{step}','model':'test-model','role':'assistant','content':[],'usage':{'input_tokens':1,'output_tokens':0}}},{'type':'content_block_start','index':0,'content_block':block},{'type':'content_block_delta','index':0,'delta':delta},{'type':'content_block_stop','index':0},{'type':'message_delta','delta':{'stop_reason':'tool_use' if item['type']=='function_call' else 'end_turn'},'usage':{'output_tokens':1}},{'type':'message_stop'}]
    ...
    events=[{'type':'chat','id':f'chat_{step}','model':'test-model','choices':[{'index':0,'delta':delta,'finish_reason':None}]},{'type':'chat','id':f'chat_{step}','model':'test-model','choices':[{'index':0,'delta':{},'finish_reason':'tool_calls' if item['type']=='function_call' else 'stop'}],'usage':{'prompt_tokens':1,'completion_tokens':1}}]
 ...
 try:
  ...
  result=subprocess.run([os.environ.get('CCSW_CODEX_BIN','codex'),'exec','--skip-git-repo-check','--sandbox','workspace-write','-c','approval_policy="never"','-c','web_search="disabled"','-C',str(p/'work'),'Read example.txt, replace before with after, and verify.'],env=env,capture_output=True,text=True,timeout=45)
  ...
  print('stderr_tail',result.stderr[-1800:])
