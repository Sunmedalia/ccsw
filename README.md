...
- 打开 Sessions 时在后台扫描；随后由日志文件变化触发更新，每 30 秒兜底检查一次。文件监听不可用时每 2 秒检查；每次只解析新增内容。统计缓存仅保留在内存中，不修改客户端日志，不保存对话正文。首次打开或重启后会重新扫描历史；删除日志后对应会话不再展示。
